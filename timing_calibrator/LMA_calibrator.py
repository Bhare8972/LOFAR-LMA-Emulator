...
class LMA_fitter_source:
    def __init__(self, LMA_data, LMA_aux, antennas_to_throw, coord_system):
        ...
        self.covariance_matrix = LMA_aux.get_covariance_matrix()
        self.max_sqrt_eig = np.sqrt( np.max( np.linalg.eigvals( self.covariance_matrix[:3,:3] ) ) )
...
class calibrator:
    def __init__(self, LMA_fname, max_redChiSquared, min_antennas_per_source, prefered_sources_per_ant, max_sources_per_run, min_sources_per_ant=3, min_antennas_per_run=6, antennas_to_throw=[], verbose=True):
        ...
        # LMA_fitters = [s for s in LMA_fitters if s.num_ants>=min_antennas_per_source and s.max_sqrt_eig<200 ]
        
        # chisq = [ fitter.max_sqrt_eig for fitter in LMA_fitters ]
    ...
    def process_second(self, second, max_num_runs, ftol=3e-16, gtol=3e-16, max_nfev=1000, do_pulse_removal=False, max_fraction_removed=0.1, min_RMS=12e-9, get_errors=False ):
        ...
        shuffle( source_list )
        source_list = sorted( source_list, key=lambda s: s.num_ants, reverse=True )[ : max_num_runs*self.max_sources_per_run ]
        # source_list = sorted( source_list, key=lambda s: s.max_sqrt_eig )[ : max_num_runs*self.max_sources_per_run ]
        shuffle(source_list)
        ...
        for run_i, (SL, numSources_per_station) in enumerate(zip(sources_for_calibration_per_run, NumSources_per_run_per_station)):
            ...
            num_parameters = number_calibrations + 4*len(SL)
            
            initial_guess = np.zeros( num_parameters, dtype=np.double )
            
            i = number_calibrations
            for s in SL:
                initial_guess[i:i+4] = s.original_XYZT
                i += 4
                
            if self.verbose:
                initial_residuals = self.workspace.loc_objective_fun( initial_guess[number_calibrations:] )
                initial_RMS = errors_to_RMS( initial_residuals, 4*len(SL) )
                print('initial RMS:', initial_RMS)
                
            ## fit location
            ret_loc_only = least_squares( self.workspace.loc_objective_fun, initial_guess[number_calibrations:], xtol=None, ftol=ftol, gtol=gtol, x_scale='jac', max_nfev=max_nfev )
            if self.verbose:
            ...
            ## fit delays now
            initial_guess[number_calibrations:] = ret_loc_only.x
            ret = least_squares(self.workspace.full_objective_fun, initial_guess, xtol=None, ftol=ftol, gtol=gtol, x_scale='jac', max_nfev=max_nfev )
            # residuals = None
                
            residuals = self.workspace.full_objective_fun( ret.x )
            RMS = errors_to_RMS( residuals, num_parameters ) 
            
            
            if self.verbose:
                print('fit RMS:', RMS, 'fit msg:', ret.message)
            
            if do_pulse_removal:
                max_removals = int( len(residuals)*max_fraction_removed )
                print("  num meas", len(residuals), max_removals)
                for x in range( max_removals ):
                    event_i, station_i, res = self.workspace.find_worse_fit( residuals )
                    print()
                    print('run', x, event_i, station_i, res)
                    # SL[ event_i ].antenna_mask[ station_i ] = False
                    self.workspace.set_event_mask( event_i, station_i, 0 )
                    ret = least_squares(self.workspace.full_objective_fun, ret.x, xtol=None, ftol=ftol, gtol=gtol, x_scale='jac', max_nfev=max_nfev )
                    
                    residuals = self.workspace.full_objective_fun( ret.x )
                    newRMS = errors_to_RMS( residuals, num_parameters)
                    improvement = (RMS-newRMS)/RMS
                    if self.verbose:
                        print('    new RMS:', newRMS, 'improvement:', improvement, 'fit msg:', ret.message)
                    RMS = newRMS
                    # if improvement < 0.01 or np.abs(res)<3.0e-8:
                    # if np.abs(res)<2.5e-8:
                    if RMS < min_RMS:
                        break  
            ...
            ## place results
            found_delays = ret.x[:number_calibrations]
            for ant_i, delay in zip(antenna_order, found_delays):
                antenna = self.antenna_names[ant_i]
                
                if self.verbose:
                    print( antenna, delay )
                    
                if antenna not in outputs:
                    outputs[antenna] = []
                outputs[antenna].append( delay )
                
            if get_errors:
                if residuals is None:
                    residuals = self.workspace.full_objective_fun( ret.x )
                    
                self.workspace.sort_residuals( residuals, station_fits, self.antenna_names )
                    
                
                    
        if get_errors:
            return outputs, station_fits
        else:
            return outputs
...
if __name__ == "__main__":
    ...
    do_pulse_removal = True
    max_fraction_removed=0.1
    min_RMS=12e-9
    
    num_runs = 10
    max_nfev = 1000
    
    out_fname = './results/COLMA_180522_030000_0600_normalCal.txt'
    ...
    if mode == 'full_run':
        ...
        for i, (second, num) in enumerate( zip(all_seconds, number_sources) ):
            ...
            outputs = CAL.process_second( second, num_runs, max_nfev=max_nfev, do_pulse_removal=do_pulse_removal, max_fraction_removed=max_fraction_removed, min_RMS=min_RMS,  )
            ...
            for sname, results in outputs.items():
                ave = np.average( results )
                N = len( results )
                STD = np.std( results )
                
                err_est = STD/np.sqrt(N)
                
                print(sname, ave, '+/-', err_est, "N:", N, 'STD:', STD)
                
                out_file.write( sname )
                out_file.write( ' ' )
                out_file.write( str(ave) )
                out_file.write( ' ' )
                out_file.write( str(err_est) )
                out_file.write( '\n' )
            print()
            print()
        ...
        plot_errors = False
        if plot_errors:
            outputs, errors_by_station = CAL.process_second( second_to_test, num_runs, max_nfev=max_nfev, max_fraction_removed=max_fraction_removed, min_RMS=min_RMS, get_errors=True )
        else:
            outputs = CAL.process_second( second_to_test, num_runs, max_nfev=max_nfev, do_pulse_removal=do_pulse_removal, max_fraction_removed=max_fraction_removed, min_RMS=min_RMS, get_errors=False )
        ...
        if plot_errors:
            for sname, errors in errors_by_station.items():
                print(sname)
